...
def massive_download_dataset(msv: str, out_dir: Path, voxel_dir: Path = None, mz_bin: float = 1.0, mz_parent_bin: float = 1.0, rt_bin_sec: float = 1.0, async_voxel: bool = False) -> None:
    """
    MassIVE exposes public datasets via anonymous FTP. Correct path is /{msv}/ccms_data/{version}.
    """
    ...
    remote_dirs = []
    # ccms-ftp layout: /vXX/MSV.../ and /vXX/MSV.../ccms_data
    for v in versions:
        remote_dirs.append(f"/{v}/{msv}")
        remote_dirs.append(f"/{v}/{msv}/ccms_data")
    # massive-ftp layout: /MSV.../ccms_data/{v} and /MSV.../ccms_data
    for v in versions:
        remote_dirs.append(f"/{msv}/ccms_data/{v}")
        remote_dirs.append(f"/{msv}/ccms_data")
...
def main():
    ...
    p2.add_argument("--voxel-dir", type=str, default=None, help="Output directory for voxel files (enables async conversion and mzML cleanup)")
    ...
    p7 = sub.add_parser("massive-download-and-voxel", help="Download and immediately convert to voxel, deleting mzML")
    p7.add_argument("--msv", required=True)
    p7.add_argument("--out", required=True)
    p7.add_argument("--voxel-dir", type=str, required=True, help="Output directory for voxel files")
    p7.add_argument("--mz-bin", type=float, default=1.0)
    p7.add_argument("--mz-parent-bin", type=float, default=1.0)
    p7.add_argument("--rt-bin-sec", type=float, default=1.0)
    p7.add_argument("--lftp-parallel", type=int, default=None, help="lftp --parallel (default: 8, overrides LFTP_PARALLEL env)")
    p7.add_argument("--lftp-pget", type=int, default=None, help="lftp --use-pget-n (default: 1, overrides LFTP_PGET env)")
    ...
    if args.cmd == "batch-download":
        ...
        # Set CLI overrides for lftp parallelism
